...
const ASSET_ROOTS = [
  ...
  // Self-hosted woff2. Referenced from fonts.css by url(), so the reference
  // walk finds them the same way it finds a photograph.
  'fonts',
];
...
# The favicon is NOT content-hashed — a browser looks for it by a fixed name,
# so it cannot be busted by renaming. A week is long enough to be free on
# repeat visits and short enough that a change is not stuck for a year.
/favicon.svg
...
# Fonts never change under their name — a different weight or subset is a
# different file — so they get the same year as the hashed code. crossorigin
# is not needed here: they are same-origin now.
/fonts/*
  Cache-Control: public, max-age=31536000, immutable
...
function main() {
  ...
  /* Root-level files that are neither code nor a walked asset root, so
     nothing above would have copied them. A favicon has to sit where the
     browser looks for it. */
  for (const rel of ['favicon.svg', 'mask-icon.svg']) {
